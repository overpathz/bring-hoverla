...
public class CacheProxyConfigurator implements ProxyConfigurator {
    ...
    private Map<String, Map<List<Object>, Object>> cache = new ConcurrentHashMap<>();

    @Override
    public Object replaceWithProxyIfNeeded(Object t, Class<?> implClass) {
        boolean isCacheable = Arrays.stream(implClass.getMethods()).anyMatch(m -> m.isAnnotationPresent(Cacheable.class));
        if (isCacheable) {
            MethodInterceptor methodInterceptor = (obj, method, args, proxy) -> {
                if (method.isAnnotationPresent(Cacheable.class)) {
                    ...
                    return getCacheData(method, args, t, cacheName);
                }
                return method.invoke(t, args);
            };
            return Enhancer.create(implClass, methodInterceptor);
        }
        return t;
    }
